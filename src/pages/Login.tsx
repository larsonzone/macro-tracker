  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try { await login(email, password); }
      ...
      setError(msg.includes('invalid-credential') || msg.includes('wrong-password') || msg.includes('user-not-found')
        ? 'Invalid email or password.' : 'Sign in failed. Please try again.');
  ...
  return (
    <div className="min-h-screen bg-app flex items-center justify-center p-4">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -left-40 w-96 h-96 rounded-full opacity-20" style={{ background: 'radial-gradient(circle, rgba(6,78,59,0.8) 0%, transparent 70%)' }} />
        <div className="absolute -bottom-40 -right-40 w-96 h-96 rounded-full opacity-15" style={{ background: 'radial-gradient(circle, rgba(212,160,23,0.4) 0%, transparent 70%)' }} />
      </div>
      ...
      <div className="w-full max-w-sm relative opacity-0 animate-scale-in" style={{ animationFillMode: 'forwards' }}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/25 text-sm font-sans mt-2 tracking-wide">Premium Nutrition Intelligence</p>
        </div>
        ...
        <div className="rounded-3xl p-7" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 24px 64px rgba(0,0,0,0.4)' }}>
          ...
          <p className="text-white/30 text-sm font-sans mb-6">Sign in to your account</p>
        ...
        <p className="text-center text-white/25 text-sm font-sans mt-6">
          No account?{' '}
          <Link to="/signup" className="text-gold-400 hover:text-gold-300 font-semibold transition-colors">Create one free</Link>
        </p>
      </div>
    </div>
  );
